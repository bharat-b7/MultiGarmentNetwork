...
def render_visibility(v, f, vt, ft, width, height, camera_f, camera_c,
                      camera_t=np.zeros(3, dtype=np.float32), camera_rt=np.zeros(3, dtype=np.float32), name=None):
    RESOLUTION = 512
    unq_verts, unq_inds = np.unique(f.ravel(), return_index=True)
    unq_vt = vt[ft.ravel()[f.ravel()[unq_inds] ]]
    unq_v = v[:,f.ravel()[unq_inds]]
    ui = np.round(vt[:, 0] * RESOLUTION - 1).astype(np.int64)
    vi = np.round((1 - vt[:, 1]) * RESOLUTION - 1).astype(np.int64)
    map = np.zeros(RESOLUTION, RESOLUTION)
    map[vi, ui] = np.array(range(len(v)))


    v, f = split_vertices_by_face(v, f)
    fids = render_colored_batch(v, f, f, width, height, camera_f, camera_c, bgcolor, 1, camera_t, camera_rt)
...
if __name__ == '__main__':
    ...
    mesh = Mesh(filename='/BS/bharat-2/work/data/renderings/renderpeople/rp_rosy_posed_016_30k/rp_rosy_posed_016_30k.obj')
    tex = cv2.imread('/BS/bharat-2/work/data/renderings/renderpeople/rp_rosy_posed_016_30k/rp_rosy_posed_016_30k_orig.jpg')[...,::-1]

    if mesh.vt.shape[1] == 3:
        mesh.vt = mesh.vt[:,:2]

    mesh.v -= mesh.v.mean(axis=0)

    # vis = render_visibility(np.array([mesh.v]), mesh.f, mesh.vt, mesh.ft, 1024, 1024, [1024, 1024], [512, 512])


    rendered = render_textured_batch([mesh.v]*3, mesh.f.astype('int32'), mesh.vt, mesh.ft.astype('int32'), tex.astype('float32')/255,
                                     1024, 1024, [1024, 1024], [512, 512],
                          bgcolor=np.zeros(3, dtype=np.float32), camera_t=np.array([0, 0, -2.2]),
                          camera_rt=np.zeros(3, dtype=np.float32))
    plt.imshow(rendered.numpy()[0])
    plt.show()
